...
var datesForMarkers = [data[data.length - 50], data[data.length - 70]];
...
for (var i = 0; i < datesForMarkers.length; i++) {
	if (i !== indexOfMinPrice) {
		markers.push({ time: datesForMarkers[i].time, position: 'inBar', color: '#e91e63', shape: 'circle', text: 'Sell @ ' + Math.floor(datesForMarkers[i].high + 2) });
	} else {
		markers.push({ time: datesForMarkers[i].time, position: 'inBar', color: '#2196F3', shape: 'circle', text: 'Buy @ ' + Math.floor(datesForMarkers[i].low - 2) });
	}
}
markers.push({ time: data[data.length - 5].time, position: 'inBar', color: '#f68410', shape: 'circle', text: 'D' });
